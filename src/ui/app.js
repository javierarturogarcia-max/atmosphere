...
export function iniciar(raiz) {
  ...
  function pintarNav() {
    ...
    // Monte Adentro vive en su propia pagina: es un juego 3D a pantalla
    // completa con su propio motor, y meterlo dentro de la carcasa de la app
    // solo le quitaria sitio.
    const enlaceJuego = el('a', {
      clase: 'nav-item',
      texto: '🌄  Monte Adentro',
      title: 'Juego 3D: el nino del campo',
      href: './juego.html',
      estilo: 'margin-top:auto;text-decoration:none',
    });
    lateral.appendChild(enlaceJuego);

    const est = almacen.get();
    lateral.appendChild(el('div', { estilo: 'padding:13px 11px 0' }, [
